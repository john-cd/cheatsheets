...
### Declarative Deployments (Kustomize)

Kustomize is built into `kubectl` and allows for template-free customization of YAML manifests.

To deploy a directory containing a `kustomization.yaml` file:

```shell
kubectl apply -k <directory-path>
```

To view the generated native manifests without applying them:
